...
const verifyOtpService = async (verifyOtpRequest: VerifyOtpRequestDTO) => {
  ...
  try {
    ...
    if (attempts >= MAX_ATTEMPTS) {
      logger.warn(`OTP brute-force lockout for email=${verifyOtpRequest.email}`);
      throw new ApiError(
    ...
    if (!storedOtp) {
      logger.warn(`OTP expired or not found for email=${verifyOtpRequest.email}`);
      throw new ApiError(ErrorCode.BAD_REQUEST, 'OTP has expired — request a new code');
    }

    // 3. Constant-time comparison to prevent timing attacks
    const isValid = timingSafeEqual(storedOtp, verifyOtpRequest.otp);
